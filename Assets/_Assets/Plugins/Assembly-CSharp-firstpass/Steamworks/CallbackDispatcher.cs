...
namespace Steamworks
{
	public static class CallbackDispatcher
	{
		...
		internal static void Register(Callback cb)
		{
			...
			Dictionary<int, List<Callback>> dictionary = ((!cb.IsGameServer) ? m_registeredCallbacks : m_registeredGameServerCallbacks);
			lock (m_sync)
			{
				List<Callback> value;
		...
		internal static void RunFrame(bool isGameServer)
		{
			...
			HSteamPipe hSteamPipe = (HSteamPipe)((!isGameServer) ? NativeMethods.SteamAPI_GetHSteamPipe() : NativeMethods.SteamGameServer_GetHSteamPipe());
			NativeMethods.SteamAPI_ManualDispatch_RunFrame(hSteamPipe);
			Dictionary<int, List<Callback>> dictionary = ((!isGameServer) ? m_registeredCallbacks : m_registeredGameServerCallbacks);
